...
def resolve_and_narrate(
    ...
    *,
    ...
    # ── 재설계 Path A: consolidate(텍스트-only 페르소나 슬롯) 경로 폐기(코드 제거 완료) ──────────
    ...
    #    name_evidence(대사)로 슬롯에 이름을 얹는다 — 얼굴(시각정체) 유지 + 명명은 대사가 담당.
    #    옛 consolidate 경로(대사만으로 슬롯 발명, 얼굴 무관)는 dead code로 제거됨(2026-07-25).
