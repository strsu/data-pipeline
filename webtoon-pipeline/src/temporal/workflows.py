...
class RegenerateCharacterWorkflow:
    ...
    - reresolve: 얼굴 이동/섞임 풀기 후. 등장 에피소드 전부 순차 재해소 후 프로필 재도출로 마무리.
                 기본은 텍스트 전용(rerun_extract=False + 옛 화자 무효화 — §20.3 개정, ~3배 절감).
                 inp.rerun_extract=True(수동 버튼 깊은 모드)면 비전까지 재실행.
            ...
            for i, ep in enumerate(episodes):
                ...
                out = await workflow.execute_activity(
                    ...
                    args=[ep["episode_id"], meta["webtoon_id"], meta["run_id"], i + 1,
                          bool(inp.rerun_extract), inp.invalidate_character_ids or []],
                    task_queue=STEP3_QUEUE,
                    # rerun_extract=True 실측 회차당 ~1.5h(§20.5), False는 ~25~35분 — 넉넉히.
...
@workflow.defn
class FaceChromaSyncWorkflow:
    """human 얼굴 교정의 실시간 Chroma 투영(T3, prd §10) — service 교정 API가 발화.

    Temporal을 경유하는 이유: 교정 API 응답은 즉시 반환하면서 투영은 재시도 가능한
    durable 작업으로 — Chroma/model-api 일시 장애가 사용자 라벨링을 막지 않는다.
    bulk는 청크(_T3_CHUNK)로 쪼개 청크당 액티비티 1개로 순차 실행 — 완료 청크는
    히스토리에 남아 재시도가 전체를 재실행하지 않고, 수백 장 bulk도 타임아웃에 안 걸린다.
    청크 최종 실패는 기록만 하고 계속 진행(멱등 + 배치 안전망: step2 시딩/퍼지/복원이
    다음 실행에서 수렴시킨다).
    """

    @workflow.run
    async def run(self, inp: FaceChromaSyncInput) -> dict:
        # patched() 가드 불요 — 이 워크플로는 청크 구조로 첫 배포된다(구 히스토리 부재).
        # 이후 청크 구조를 바꿀 땐 in-flight가 초~분 수명임을 감안해 patched() 도입 검토.
        ids = inp.face_detection_ids or []
        totals: dict = {}
        failed_chunks = 0
        for start in range(0, len(ids), _T3_CHUNK):
            chunk = ids[start:start + _T3_CHUNK]
            try:
                r = await workflow.execute_activity(
                    activities.face_chroma_sync,
                    replace(inp, face_detection_ids=chunk),
                    task_queue=ORCH_QUEUE,
                ...
                failed_chunks += 1
                workflow.logger.warning(
                    "[t3-sync] webtoon=%s 청크 %d~%d 최종 실패 — 배치 안전망이 수렴",
                    inp.webtoon_id, start, start + len(chunk) - 1,
                )
        if failed_chunks:
            totals["failed_chunks"] = failed_chunks
        return totals
