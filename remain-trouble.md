...
### D3. **429-drop 버그** (선결 블로커)
- `call_llm_json`이 429(4xx) 재시도 안 함 → 세그 비전콜 429시 **세그 통째 드롭=데이터 손실**. 터널이든 직결이든 GLM 상단(api.z.ai)이 CONC2에도 레이트리밋. **429 백오프 재시도 추가** 필요(세그-단위 step3의 실질 블로커).
